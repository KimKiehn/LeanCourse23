...
/-
In this file the notion of a cone in an arbitrary ring is introduced. It generalizes the
notion of a positive cone in the sense that we do not have antisymmetry.
For prime cones the antisymmetry can later one be ensured by introducing the support of
a cone. This will be a prime ideal which can be modded out and then the prime cone
will induce an ordering on the quotient.
Furthermore, we describe properties of cones, cones generated by a set of elements
and in general the existence of maximal and prime cones.
...
lemma Cone.ext_neq_iff_neq (C D: Cone R): C≠ D ↔ C.nonneg ≠ D.nonneg:=by{
  constructor
  · intro h
    by_contra hcon
    exact h ((Cone.ext_iff C D).mpr hcon)
  · intro h
    by_contra hcon
    exact h ((Cone.ext_iff C D).mp hcon)
}
...
lemma Cone.generated_by.subset (I: Set R): I⊆ (Cone.generated_by I).nonneg:=by{
  intro x hx
  apply AddSubmonoid.subset_closure
  apply Submonoid.subset_closure
  left
...
(Cone.generated_by C.nonneg).nonneg= C.nonneg:=by{
  ...
  exact Subset.antisymm hle (Cone.generated_by.subset C.nonneg)
}
...
/-
The following is an explicit description of the cone generated by a
cone and one further element. This will later one simplify the proof that
maximal cones are prime.
-/
...
/-
as menstioned before the proof of this theorem uses the explicit description of
cones extended by a single element, namely for a maximal cone one assumes that it is
not prime extends it in two ways by the witnesses for the non-primeness of it in order
to seek a contradiction via a simple computation.
-/
...
-- The following proof is a standard Zorn's lemma argument
